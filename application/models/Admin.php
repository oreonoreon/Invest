...
class Admin extends Model {
	...
	public function withdrawRefList() {
		...
		if (!empty($result)) {
			foreach ($result as $key => $val) {
				...
				$account = $this->db->row('SELECT login, wallet, walletYandexMoney FROM accounts WHERE id = :id', $params)[0]; // добавил , walletYandexMoney чтобы выводить в админку
				$arr[$key]['login'] = $account['login'];
				$arr[$key]['wallet'] = $account['wallet'];
                                $arr[$key]['walletYandexMoney'] = $account['walletYandexMoney']; // добавил $arr[$key]['walletYandexMoney'] = $account['walletYandexMoney']; чтобы выводить в админку
			}
		}
		return $arr;
	}
	...
	public function withdrawRefComplete($id) {
		...
		$params = [
			...
			'uid' => $data['uid'],
			'unixTime' => time(),
			'description' => 'Выплата реферального вознаграждения произведена, сумма '.$data['amount'].' RUB',
                        'descriptionfordull' =>'Выплата реферального вознаграждения произведена, сумма '.$data['amount'].' RUB',
		];
	...
	public function withdrawTariffsComplete($id) {
		...
		$params = [
			...
			'uid' => $data['uid'],
			'unixTime' => time(),
			'description' => 'Выплата по тарифу # '.$id.' произведена, сумма '.$data['sumOut'].' RUB',
                        'descriptionfordull' => 'Выплата по тарифу произведена, сумма '.$data['sumOut'].' RUB',
		];
		$this->db->query('INSERT INTO history VALUES (:id, :uid, :unixTime, :description, :descriptionfordull)', $params);
		return true;
	}
